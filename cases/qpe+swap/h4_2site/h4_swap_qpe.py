...
if not flag_test_qpe:
    ...
    start_time = datetime.now()
    simulation_results_filename = "h4_sim_ovlp2_" + start_time.strftime("%Y-%m-%d-%H-%M-%S")  + ".json"
    ...
    with open(simulation_results_filename, 'w') as f: 
        json.dump(result.get_memory(t_circ), f, indent=2)
    #print(memory)
    ...
    #     if int(write_time.seconds / 3600) > 0:
    #         simulation_results_file.close()
    #         start_time = datetime.now()
    #         simulation_results_filename_new = 'h4_sim_ovlp1.txt_' + str(start_time)
    #         shutil.copy2(simulation_results_filename, simulation_results_filename_new)
    #         simulation_results_file = open(simulation_results_filename_new, 'a')
    #         simulation_results_filename = simulation_results_filename_new

    #     #simulation_results_file.write("%8d\t%12.8e\n" % (ib, iovlp))
    #     simulation_results_file.write("%12.6e\n" % iovlp)
    #     simulation_results_file.close()
    # -------------- Old implementation --------------------- #
