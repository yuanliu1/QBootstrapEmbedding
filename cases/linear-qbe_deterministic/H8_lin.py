...
def get_perturbed_gs(potential_coeffs, fragment):
    ...
    if fragment == '0':
        ...
        potential_term = construct_update_matrix(np.array([[0,0],[0,1]]), (0,2)) + construct_update_matrix(np.array([[0,0],[0,1]]), (8,0))
        ...
        potential_term = construct_update_matrix(np.array([[0,0],[0,1]]), (0,2)) + construct_update_matrix(np.array([[0,0],[0,1]]), (8,0))
        H += potential_coeffs[10]*potential_term
    elif fragment == '6':
        potential_term = construct_update_matrix(np.array([[0,0],[0,1]]), (0,0)) + construct_update_matrix(np.array([[0,0],[0,1]]), (6,0))
        H = fragment_hamiltonians[fragment] + potential_coeffs[11]*potential_term
...
gs_filename = 'gs_vec_lin_run00.pkl'
...
#             print('fragment num', f, len(gs))
        ...
        # save the fragment_gs for all fragments, 12/12/2022
...
max_runs = 0 # number of runs, i.e. number of times the algorithm loops through all the fragments
lr_initial = 1 # initial gradient descent learning rate

lr_exponent = 25

thresh = 1e-8 # threshold for convergence, algorithm stops when the average gradient of the lagrangian wrt the lagrange multipliers falls below this value
...
run, rmse, energy_error, complete = main_algorithm_linear(max_runs, num_iter, lr_initial, 
                                              lr_exponent, thresh, quit_if_diverge = False)
#np.save('rmse_H8_lin.npy', rmse)
#np.save('energy_error_H8_lin.npy', energy_error)
